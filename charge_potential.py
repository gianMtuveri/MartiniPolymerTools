...
from scipy.constants import elementary_charge as e_charge, epsilon_0
...
# === Accumulate charge and potential ===
zeta_list = []
phi_list = []
...
for ts in tqdm(u.trajectory[-1000:], desc="Processing frames"):
    ...
    # Solve Poisson’s equation (spherical symmetry)
    integrand = rho_r * r_centers**2
    # Avoid division by zero at r=0
    E_r = cumtrapz(integrand, r_centers, initial=0)
    with np.errstate(divide='ignore', invalid='ignore'):
        E_r = np.where(r_centers != 0, E_r / (epsilon * r_centers**2), 0)
...
phi_mean = np.mean(phi_list, axis=0)  # Average potential
...
ax1.plot(r_centers * 1e9, rho_avg * 1e3, label="ρ(r)", color=color1, lw=6)
...
ax2.plot(r_centers * 1e9, phi_mean * 1e3, label="φ(r)", color=color2, lw=6)
...
zeta_potential_mv = phi_mean[idx_slip] * 1e3
print(f"Zeta potential at {slip_radius_nm} nm: {zeta_potential_mv:.2f} mV")
